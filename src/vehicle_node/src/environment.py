...
class Environments(object):
    def __init__(self, course_idx, dt=0.1, min_num_agent=8):
        ...
        self.target_path = []  # target_path를 클래스 변수로 추가
    ...
    def initialize(self, init_num=6):
        ...
        for i in range(init_num):
            if i == 0:
                ...
                self.target_path = self.map_pt[CourseList[self.course_idx][0]]  # 경로를 self.target_path에 저장
    ...
    def run(self):
        target_path = self.target_path  # 클래스 변수에서 target_path를 가져옴
        # print(target_path)
        for id_ in self.vehicles.keys():
            vehicle = self.vehicles[id_]
            sensor_info = vehicle.get_measure(self.vehicles)
            pose = {'x': vehicle.x, 'y': vehicle.y, 'theta': vehicle.theta}
            local_path = create_local_path(sensor_info)
            global_path = convert_local_to_global(local_path, pose)
            filtered = apply_kalman_filter(global_path)
            # print(filtered)

            action = determine_possible_paths(vehicle, filtered)
            if action == 'stop':
